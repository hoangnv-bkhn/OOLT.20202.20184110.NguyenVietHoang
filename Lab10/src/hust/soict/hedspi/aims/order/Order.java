...
public class Order {
	...
	public void addMedia(Media media) throws LimitExceededException {
		if (itemsOrdered.size() >=10) {
			...
			throw new LimitExceededException("Error!!! The order list is full");
		}else {
	...
	public void addMedia(Media ...medias) throws LimitExceededException {
		if (medias == null) {
			...
			throw new NullPointerException("Error!!! There is no media to add.");
			...
			if (itemsOrdered.size() + n > 10) {
				...
				throw new LimitExceededException("There is not enough slot left for these items.");
			}else {
	...
	public String printDetalsString() {
		String s = "******************Order******************\n";
		s += "Date: \n";
		s += dateOrdered.print()+"\n";
		s += "Ordered items:\n";
		for (int i = 0; i < itemsOrdered.size(); i++) {
			s += "\t"+itemsOrdered.get(i).getTitle()+" "+itemsOrdered.get(i).getCategory()+" "+": "+itemsOrdered.get(i).getCost()+"$"+"\n";
		}
