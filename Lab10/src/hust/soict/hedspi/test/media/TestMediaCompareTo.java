...
public class TestMediaCompareTo {
	...
	// test compareTo for DVD, CD, Book (they extent title from Media)
		...
		try {
			cd1 = new CompactDisc(4, "Puss in Boots", "Fairy Tale", 10, "Folk");
		} catch (InvalidAttributesException e) {
			e.printStackTrace();
		}
		CompactDisc cd2 = null;
		try {
			cd2 = new CompactDisc(4, "Rumpelstiltskin", "Fairy Tale", 10, "Folk");
		} catch (InvalidAttributesException e) {
			e.printStackTrace();
		}
	...
	// test sort Track
		...
		for (int i = 0; i < 4; i++) {
			Track track = null;
			try {
				track = new Track("Track no."+i, 10*(int)Math.floor(Math.random()*(10-1+1)+1));
			} catch (InvalidAttributesException e) {
				e.printStackTrace();
			}
